...
func (repo *DBRepo) AllHosts(w http.ResponseWriter, r *http.Request) {
	...
	// send all hosts data to template
	vars := make(jet.VarMap)
...
// PostHost handles posting of host form and saving into DB
...
func (repo *DBRepo) ToggleServiceForHost(w http.ResponseWriter, r *http.Request) {
	...
	// add/remove host servic from schedule
	if active == 1 {
		// add to schedule
...
func (repo *DBRepo) ToggleMonitoring(w http.ResponseWriter, r *http.Request) {
	...
	if enabled == "1" {
		...
		// remove all ites in map from schedule
		for _, entry := range repo.App.MonitorMap {
			repo.App.Scheduler.Remove(entry)
		}
